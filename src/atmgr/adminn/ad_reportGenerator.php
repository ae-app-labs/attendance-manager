...
function doClassReport(f){
	...
	var deptId    = f.lstDept.options[f.lstDept.selectedIndex].value;

	var pageCall = reportPage + "?generate=report&type=class&classId=" + classId+ "&deptId="+deptId+"&dateStart=" + dateStart + "&dateEnd=" +dateEnd + "&r="+r;
//	alert(pageCall);
	popUpWindow(pageCall, 25, 0, 650, 500);
...
<div id="staff-content">
	  ...
	  <p>The date format must be strictly followed as it is stored in the database. The format is (DD-MMM-YYYY) or (DD-Mon-YYYY). Please Use the pop up calendar.</p>
	  ...
	  <form id="form2" name="form2" method="get" style="background-color:#f4f4f4; padding:6px;" 
	  		onsubmit="return doStudentReport(this.form);">
